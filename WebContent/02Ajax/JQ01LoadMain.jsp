...
/*
  	...
  	1.$.load()
  		...
  		형식] $.load('파일경로',파라미터)
 */
//js함수 내부애서 jquery코드를 사용하는게 가능하다.
var loadCode =function(){
	//load사용시 파라미터가 필요없는 경우 생략이 가능하다.
	$('#loadView').load('./common/01LoadSub.jsp');
	alert('문서가 로드 되었습니다.');
}
$(function(){
	$('#loadTxtBtn').click(function(){
		//외부의 텍스트 파일을 현재문서로 로드한다.
		$('#loadTxt').load('./common/01LoadTxt.txt');
		alert('txt문서가 로드 되었습니다.');
	});
	$('#loadParamBtn').click(function(){
		...
		//load메소드 호출시 두번째 인자로 파라미터를 전달한다.
		$('#loadParam').load('./common/01LoadParam.jsp',params);
		//전달된 파리미터 따라 다른결과가 출력된다.
