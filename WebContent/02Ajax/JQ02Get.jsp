...
jquery에서 제공하는 ajex관련 메소드
	2. $.get()
	 ...
	 형식] $.get(url,param,callback);
	 	...
	 	-callback: 요청이 성공했을때 실행되는 콜백 메소드
	 	콜백 메소드가 호출될때 전달되는 파라미터를 통해
	 	성공 혹은 시패여부를 판단 할 수 있다
...
$(function(){
	...
	$.get(//요청 url
		...
		function(data){//콜백 메소드
			//요청 성공시 xml파일 내용을 파라미터를 통해 전달해준다
		//콜백 해준 내용을 대상으로 파싱을 진행한다
		/*
		find()를 통해 xml파일의 엘리먼트를 찾는다
		attr()메소드를 통해 엘리먼트 속성에 접근
		text()를 통해 ㅐㅌ그 사이의 값에 접근한다.
		*/
			$(data).find("명함").each(function(){
				...
				/*
				해당 여역의 내용을 empty로 삭제한 후 append로 내용을
				추가한다 html과 동일한 기능이 수행된다
				*/
				$("#xmlDisplay").empty();
	...
	사용하게 된다. 만약 폼 값이 많아서 기술이 힘든 경우에는 
